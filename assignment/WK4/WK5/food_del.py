class DeliveryPartner:
    ...
    def display(self):
        print(f"{self.name} ({self.partner_id}) | Deliveries: {self.deliveries} | Earnings: NPR {self.total_earning()}")
...
highest = partners[0]
for p in partners:
    if p.total_earning() > highest.total_earning():
        highest = p

print(f"\nHighest earning: {highest.name} — NPR {highest.total_earning()}")
