def atm(account_id, pin, action, amount=0):
    ...
    if action == "balance":
        print(
            f"{accounts[account_id]['name']} — Current Balance: NPR {accounts[account_id]['balance']}")

    elif action == "deposit":
        accounts[account_id]["balance"] += amount
        print(
            f"Deposit successful. New Balance: NPR {accounts[account_id]['balance']}")
        ...
        if amount > accounts[account_id]["balance"]:
            ...
            accounts[account_id]["balance"] -= amount
            print(
                f"Withdrawal successful. New Balance: NPR {accounts[account_id]['balance']}")
